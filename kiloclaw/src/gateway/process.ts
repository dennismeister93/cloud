...
export async function ensureOpenClawGateway(sandbox: Sandbox, env: KiloClawEnv): Promise<Process> {
  ...
  try {
    console.log('[openclaw] Waiting for OpenClaw gateway to be ready on port', OPENCLAW_PORT);
    await process.waitForPort(OPENCLAW_PORT, { mode: 'tcp', timeout: STARTUP_TIMEOUT_MS });
    console.log('[openclaw] OpenClaw gateway is ready!');

    const logs = await process.getLogs();
    if (logs.stdout) console.log('[openclaw] stdout:', logs.stdout);
    if (logs.stderr) console.log('[openclaw] stderr:', logs.stderr);
  } catch (e) {
    console.error('[openclaw] waitForPort failed:', e);
    try {
      const logs = await process.getLogs();
      console.error('[openclaw] startup failed. Stderr:', logs.stderr);
      console.error('[openclaw] startup failed. Stdout:', logs.stdout);
      throw new Error(`OpenClaw gateway failed to start. Stderr: ${logs.stderr || '(empty)'}`, {
        cause: e,
      });
    } catch (logErr) {
      console.error('[openclaw] Failed to get logs:', logErr);
      throw e;
    }
  }

  // Verify gateway is actually responding
  console.log('[openclaw] Verifying gateway health...');
