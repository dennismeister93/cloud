...
describe('needsResumeConfigModal', () => {
  ...
  it('returns true for legacy web session (has cloud_agent_session_id but no last_model) without stored config', () => {
    // Legacy sessions have cloud_agent_session_id but no last_model
    const dbSession = createDbSession({
  ...
  it('returns false for modern web session (has both cloud_agent_session_id AND last_model)', () => {
    // Modern sessions have both cloud_agent_session_id and last_model
    const dbSession = createDbSession({
      cloud_agent_session_id: 'agent_abc123',
