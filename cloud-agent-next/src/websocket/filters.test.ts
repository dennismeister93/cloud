...
describe('WebSocket Filters', () => {
  describe('parseStreamFilters', () => {
    ...
    it('should parse executionIds as comma-separated list', () => {
      const url = new URL('https://example.com/stream?executionIds=msg_1,msg_2,msg_3');
      const filters = parseStreamFilters(url, sessionId);

      expect(filters.executionIds).toEqual(['msg_1', 'msg_2', 'msg_3']);
    });
    ...
    it('should parse all filters together', () => {
      const url = new URL(
        'https://example.com/stream?fromId=100&executionIds=msg_1,msg_2&eventTypes=output&startTime=1000&endTime=2000'
      );
      const filters = parseStreamFilters(url, sessionId);
