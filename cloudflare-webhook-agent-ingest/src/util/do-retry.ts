 ...
 *   () => env.CATCHER_DO.get(env.CATCHER_DO.idFromName(key)),
 *   (stub) => stub.configure(namespace, catcherId),
 *   'configure'
 * );
 * ```
 */
  ...
  for (let attempt = 0; attempt < config.maxAttempts; attempt++) {
    try {
      ...
      if (!isRetryableError(error)) {
        logger.warn('DO operation failed with non-retryable error', {
      ...
      if (attempt + 1 >= config.maxAttempts) {
        logger.error('DO operation failed after all retry attempts', {
          operation: operationName,
          attempts: attempt + 1,
          error: lastError.message,
        });
      ...
      logger.warn('DO operation failed, retrying', {
        operation: operationName,
        attempt: attempt + 1,
